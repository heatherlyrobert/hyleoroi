...
DRAW_globals       (void)
{
   ...
   /*---(set globals)--------------------*/
   yLOG_snote   ("title");
   strcpy (my.title, "hyleoroi -- tree data structure visualization");
   yLOG_snote   ("sizes");
   my.win_h        =  700;
   my.win_w        =  700;
   my.tex_h        = 2400;
...
{
   /*---(local variables)--+-----------+-*/
   int         x_near      = 0.0;           /* x-coord of near side           */
   int         x_far       = 0.0;           /* x-coord of outer side          */
   int         x_text      = 0.0;           /* x-coord of text beginning      */
   int         y_beg       = 0.0;           /* y-coord of starting edge       */
   int         y_end       = 0.0;           /* y-coord of ending edge         */
   int         y_text      = 0.0;           /* y-coord of text middle         */
   char        t           [100];
   /*---(header)-------------------------*/
   DEBUG_GRAF   yLOG_senter  (__FUNCTION__);
   /*---(defense)------------------------*/
   ...
   /*---(prepare)------------------------*/
   x_near = my.orig_x;
   x_far  = my.orig_x + my.thick [a_node->level];
   if (a_node->level > 0)  x_text = my.orig_x + my.thick [a_node->level - 1] + 10;
   else                    x_text = x_near + 10;
   y_beg  = my.orig_y + a_node->beg;
   y_end  = my.orig_y + a_node->end;
   y_text = (y_beg + y_end) / 2.0;
   DEBUG_GRAF   yLOG_svalue  ("far", x_far);
   DEBUG_GRAF   yLOG_svalue  ("beg", y_beg);
   DEBUG_GRAF   yLOG_svalue  ("end", y_end);
   switch (my.label [a_node->level]) {
   case 'n' : strcpy (t     , a_node->name );   break;
   case 'd' : strcpy (t     , a_node->desc );   break;
   ...
   glBegin (GL_POLYGON); {
      ...
      glTranslatef ( 0.0,  0.0, - (a_node->level * 20));
      /*---(draw)------------------------*/
      glVertex3f ( x_near, y_beg, 0.0f);
      glVertex3f ( x_far , y_beg, 0.0f);
      glVertex3f ( x_far , y_end, 0.0f);
      glVertex3f ( x_near, y_end, 0.0f);
      /*---(done)------------------------*/
   } glEnd();
   /*---(label)--------------------------*/
   if (a_node->width >= 10) {
...
{
   /*---(locals)-----------+-----------+-*/
   float       x_width     = my.win_w  / 2.0;
   float       x_height    = my.win_h  / 2.0;
   /*---(header)-------------------------*/
   DEBUG_GRAF   yLOG_enter   (__FUNCTION__);
   if (a_h == 0)  a_h = 1;
   /*---(settings)-----------------------*/
   glViewport            (0,  0, my.win_w, my.win_h);
   /*> glViewport            (-x_width,  -x_height, my.win_w, my.win_h);              <*/
   glMatrixMode          (GL_PROJECTION);
   glLoadIdentity        ();
   glOrtho               (  -x_width,   x_width,  -x_height,   x_height, -1500.0,  1500.0);
   /*> gluPerspective        (45.0f, (GLfloat) my.win_w / (GLfloat) my.win_h, 0.1f, 10000.0f);   <*/
   glMatrixMode          (GL_MODELVIEW);
   glLoadIdentity        ();
   /*---(complete)-----------------------*/
   DEBUG_GRAF   yLOG_exit    (__FUNCTION__);
 ...
 *>    /+> XFillArc(DISP, bounds, gc, my.win_w / 2, my.win_h / 2, my.win_w / 2, my.win_h / 2, 0, 360 * 64);   <+/   <* 
 ...
 *>    XFillRectangle(DISP, bounds, gc, 0, 0, my.win_w, my.win_h);                                            <* 
 *>    XSetForeground(DISP, gc, 1);                                                                     <* 
 *>    /+> XFillArc(DISP, bounds, gc, win_w / 2, my.win_h / 2, win_w / 2, my.win_h / 2, 0, 360 * 64);   <+/   <* 
