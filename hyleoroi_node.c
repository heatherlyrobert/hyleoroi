...
int         s_lines     =    0;             /* input line count               */
char        s_recd      [MAX_RECD];         /* input record                   */
...
{  /*---(local variables)--+-----------+-*/
   ...
   while (1) {
      if (s_proc == 'y') {
         ...
         rcs    = fgets (s_recd, MAX_RECD, stdin);
         if (rcs == NULL) {
      ...
      if (my.chaos == 'y')   x_curr->color = rand() % n_color;
      else {
         if (my.color >= n_color)  my.color -= n_color;
